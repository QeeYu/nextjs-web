/**
 * 旅程横向滚动区块（GSAP ScrollTrigger 驱动）
 * - 水平滚动 + Pin 固定
 * - 进度条随滚动填充
 * - 面板内元素错峰入场
 * - 数字计数器动画
      ...
      return (
        <>
          ...
          <div className="j-shape absolute right-10 bottom-12">
            <div
              className="anim-blob h-2.5 w-2.5 rounded-full bg-neon/80"
      ...
      return (
        <>
          ...
          <div className="j-shape absolute right-[14%] bottom-[20%]">
            <div
              className="anim-blob h-2.5 w-2.5 rounded-full bg-pink/90"
      ...
      return (
        <>
          ...
          <div className="j-shape absolute right-10 top-20">
            <div
              className="anim-blob h-2.5 w-2.5 rounded-full bg-cyan/80"
              style={{ animationDelay: ".8s" }}
            />
          </div>
          <div className="j-shape absolute bottom-16 right-16">
            <div
              className="anim-blob h-3.5 w-3.5 rounded-full bg-pink/70"
...
export default function JourneySection({
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // ---- 数字计数器 ----
      gsap.utils.toArray<HTMLElement>(wrapRef.current!.querySelectorAll(".j-num")).forEach(
        (numEl) => {
