...
export default function ScoreTrack({
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-white/[0.02]" ref={wrapRef}>
      {/* 指示框 */}
      <div
        className={`pointer-events-none absolute left-1/2 top-1/2 z-10 h-[88%] w-[80px] -translate-x-1/2 -translate-y-1/2 rounded-lg border-[3px] transition-all duration-300 ${
      ...
      {/* 音符带 */}
      <div
        className="absolute top-1/2 -translate-y-1/2 transition-transform duration-300 ease-out"
            ...
            return (
              <div
                ...
                className={`flex h-[76px] w-[72px] flex-shrink-0 flex-col items-center justify-center rounded-xl border-2 transition-all duration-200 ${
                  isDone
                    ? "border-lime/40 bg-lime/10 opacity-40"
                    : isTarget && n.isRest
                    ? "border-dim/40 bg-white/5"
                    : isTarget
                    ? `anim-target-pulse border-cyan bg-cyan/15 ${wrongFlash > 0 ? "anim-note-shake" : ""}`
