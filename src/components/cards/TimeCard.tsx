...
export default function TimeCard() {
  ...
  return (
    <TiltCard className="card-glass card-line flex h-full flex-col p-6 md:p-7">
            ...
            <span
              ...
              onMouseEnter={() => {
                ...
                anime({
                  ...
                  color: [
                    { value: "#38e1ff", duration: 150 },
                    { value: c === ":" ? "#7c5cff" : "#e8ecff", duration: 600 },
                  ],
                });
              }}
              className={`inline-block cursor-default select-none ${
                c === ":" ? "text-neon" : "text-mist"
              }`}
            >
              {c}
            </span>
      ...
      <p
        onMouseEnter={wob}
        className="mt-3 cursor-default text-center text-sm text-dim"
      >
        {now
          ? `${now.getFullYear()} 年 ${pad(now.getMonth() + 1)} 月 ${pad(
              now.getDate()
            )} 日`
          : "— — —"}{" "}
        · 星期{WEEK[now?.getDay() ?? 0]}
      </p>
      <p onMouseEnter={wob} className="mt-1 cursor-default text-center text-sm text-cyan">
        {hour < 5
          ? "深夜好，夜猫子 🦉"
          : hour < 11
          ? "早上好，新的一天 ☀️"
      ...
      <div className="mt-4">
        <div className="relative h-2 rounded-full bg-white/8">
          <div
            className="absolute left-0 top-0 h-full rounded-full bg-linear-to-r from-cyan/60 to-cyan/30 transition-[width] duration-1000"
            style={{ width: `${dayPct}%` }}
          />
          <div
            className="absolute top-1/2 h-3 w-3 -translate-y-1/2 rounded-full bg-cyan shadow-[0_0_8px_rgba(56,225,255,0.8)] transition-all duration-1000"
            style={{ left: `${dayPct}%` }}
          />
        </div>
      ...
      <div className="mt-auto pt-4">
        ...
        <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white/10">
          <div
            className="h-full rounded-full bg-linear-to-r from-cyan to-pink transition-[width] duration-1000"
            style={{ width: `${dayPct}%` }}
          />
        </div>
